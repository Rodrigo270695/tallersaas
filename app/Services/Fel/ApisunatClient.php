...
final class ApisunatClient
{
    ...
    public function generarComprobante(array $credenciales, array $payload): array
    {
        ...
        try {
            ...
            $json['_tallersaas_emission_mode'] = $credenciales['mode'];
    ...
    public function construirPayload(
        ...
        $igvPct = number_format((float) $setting->igv_porcentaje, 0, '.', '');
        ...
        $items = $venta->lineas->map(function (VentaLinea $ln) use ($igvPct): array {
            ...
            return [
                ...
                'descripcion' => mb_substr((string) $ln->descripcion, 0, 250),
        ...
        return [
            ...
            'cliente_direccion' => mb_substr((string) ($receptor['direccion'] ?? '-'), 0, 250) ?: '-',
            'items' => $items,
