...
/**
 * Consulta DNI/RUC vía APISUNAT — respaldo cuando apiperu.dev falla.
 *
    ...
    private function getJson(string $path): array
    {
        ...
        if ($token === null) {
            throw new ApiPeruConsultaException(
                'No hay un token APISUNAT configurado como respaldo.',
                503,
                'not_configured',
            );
        }
        ...
        if ($response->status() === 429) {
            throw new ApiPeruConsultaException(
                'APISUNAT alcanzó el límite de consultas. Intenta de nuevo en un momento.',
                429,
                'rate_limit',
            );
        }
        ...
        if ($response->status() === 401) {
            throw new ApiPeruConsultaException(
                'El token APISUNAT de respaldo no es válido.',
                503,
                'not_configured',
            );
        }
        ...
        if (! $response->successful()) {
            ...
            throw new ApiPeruConsultaException(
                "APISUNAT respondió con un error inesperado (HTTP {$response->status()}).",
