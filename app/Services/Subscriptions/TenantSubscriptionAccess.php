...
final class TenantSubscriptionAccess
{
    ...
    public function resolveDenial(Tenant $tenant): ?string
    {
        ...
        if ($subscription instanceof Subscription) {
            ...
            if ($subscription->estado === 'grace') {
                return $this->isWithinGraceWindow($subscription) ? null : self::DENIAL_EXPIRED;
            }

            if ($subscription->estado === 'trial') {
                $trialEnd = $subscription->trial_ends_at ?? $tenant->trial_ends_at;

                return $this->isPast($trialEnd) ? self::DENIAL_EXPIRED : null;
            }
    ...
    {
        return match ($denial) {
            self::DENIAL_CANCELLED => 'Este taller fue cancelado. Contacta a soporte si crees que es un error.',
            self::DENIAL_SUSPENDED => 'El acceso está suspendido por falta de pago. Renueva tu plan en Orvae para reactivar la cuenta.',
            self::DENIAL_EXPIRED => 'El plan de tu taller ha vencido. Renueva tu suscripción en Orvae para volver a ingresar.',
            default => 'No puedes acceder a este taller en este momento.',
        };
    }
