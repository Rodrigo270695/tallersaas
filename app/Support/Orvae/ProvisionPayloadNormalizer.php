...
/**
 * Adapta el JSON anidado de Orvae (checkout / Aula Virtual) al contrato
 * plano que espera {@see ProvisionTenantRequest}.
 *
 * Si el payload ya llega plano (`tenant_slug`, `admin_email`, ...) se
 * devuelve tal cual: esto permite pruebas manuales con curl sin tener
 * que envolver todo en `{tenant: {...}, customer: {...}}`.
 */
final class ProvisionPayloadNormalizer
{
    ...
    public static function normalize(array $input): array
    {
        ...
        if ($slug === '') {
            $slug = Str::slug((string) ($tenant['name'] ?? 'taller'));
        }
        ...
        return array_merge($input, [
            ...
            'plan_slug' => $planSlug,
            'tenant_slug' => $slug,
            'razon_social' => trim((string) ($tenant['name'] ?? 'Taller mecánico')),
