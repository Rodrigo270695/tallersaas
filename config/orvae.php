...
return [
    ...
    /*
    |--------------------------------------------------------------------------
    | Integración Orvae PE -> TallerSaaS
    |--------------------------------------------------------------------------
    |
    | Mismo patrón usado entre Orvae PE y VetSaaS: Orvae PE hace POST al
    | endpoint interno de TallerSaaS firmado con HMAC-SHA256 sobre
    |     "{timestamp}.{raw_body}"
    | Cabeceras esperadas:
    ...
    */
    'tenant' => [
        'scheme' => env('TALLERSAAS_TENANT_SCHEME', 'https'),
        // Deprecado: usar config('tenant.root_domain'). Se mantiene por compatibilidad.
        'domain' => env('TALLERSAAS_TENANT_DOMAIN', env('TENANT_ROOT_DOMAIN', 'tallersaas.orvae.pe')),
        'login_path' => env('TALLERSAAS_TENANT_LOGIN_PATH', '/login'),
        'bootstrap_ttl_hours' => (int) env('TALLERSAAS_BOOTSTRAP_TTL_HOURS', 48),
    ],
];
