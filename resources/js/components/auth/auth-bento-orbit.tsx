import { MessageCircle, Receipt, ShieldCheck, Wrench } from 'lucide-react';
...
function BentoCard({ icon: Icon, eyebrow, title, accent, className }: BentoCardProps) {
    return (
        <div
            aria-hidden="true"
            className={cn(
                'animate-in fade-in slide-in-from-bottom-3 absolute w-60 rounded-2xl border border-border/60 bg-card/80 p-4 text-left shadow-[0_20px_60px_-30px_rgba(60,20,0,0.35)] backdrop-blur-xl duration-700 ease-out dark:bg-card/60 dark:shadow-[0_20px_60px_-30px_rgba(0,0,0,0.7)]',
                className,
            )}
        >
    ...
    return (
        ...
        >
            <BentoCard
                ...
                title="Del ingreso a la entrega"
                accent={
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        {['Diagnóstico', 'Repuestos', 'Mano de obra', 'Entrega'].map((chip) => (
                            <span
                                key={chip}
            ...
            <BentoCard
                ...
                accent={
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-success/12 px-1.5 py-0.5 text-[0.7rem] font-medium text-success">
                        Avisa cuando el vehículo está listo
                    </span>
                }
