...
export type FilterChip<TValue extends string> = {
    value: TValue;
    label: string;
    /** Conteo opcional al lado de la etiqueta (ej. "Activos (13)"). */
    count?: number;
    icon?: ReactNode;
...
function inferTone(value: string, explicit?: FilterChipTone): FilterChipTone {
    ...
    if (['activa', 'activo', 'activas', 'activos', 'completada', 'completado', 'confirmada', 'abierta', 'pagada', 'alta', 'emitida'].includes(v)) {
        return 'success';
    }

    if (['inactiva', 'inactivo', 'inactivas', 'inactivos', 'cancelada', 'cancelado', 'anulada', 'cerrada'].includes(v)) {
        return 'muted';
    }
