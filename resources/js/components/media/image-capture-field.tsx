 ...
 * - Solo «Elegir archivo» en laptop/PC
...
export function ImageCaptureField({
    ...
    const applyFile = async (file: File | null | undefined) => {
        ...
        if (!file.type.startsWith('image/') && file.type !== '') {
            toastManager.error({
                title: 'El archivo debe ser una imagen (JPG, PNG o WebP).',
            });
        ...
        try {
            ...
            if (compressed.size > 2_000_000) {
                toastManager.error({
                    title: 'La foto sigue pesando demasiado. Prueba con otra.',
                });
            ...
            toastManager.error({
                title: 'No se pudo procesar la foto. Inténtalo de nuevo.',
            });
        } finally {
                    ...
                    <div className="relative size-20 shrink-0 overflow-hidden rounded-lg border border-border shadow-sm">
                        ...
                        {value instanceof File ? (
                            <Button
                                ...
                                size="icon"
                                variant="destructive"
                                className="absolute bottom-1 right-1 size-7 cursor-pointer shadow-md"
                                disabled={busy}
                                title="Quitar foto"
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-2">
                    ...
                    >
                        ...
                        >
                            ...
                            {value || existingUrl ? 'Cambiar foto' : 'Elegir archivo'}
                        </Button>
                    )}
                    ...
                    <p className="text-[11px] leading-snug text-muted-foreground">
                        {compressing
                            ? 'Comprimiendo foto…'
                            : 'Opcional. Si la foto pesa mucho (p. ej. iPhone), se comprime sola antes de guardar.'}
                    </p>
                    {value instanceof File ? (
                        <p className="text-[11px] tabular-nums text-muted-foreground">
                            Lista ({formatKb(value.size)})
                        </p>
                    ) : null}
                </div>
            </div>
