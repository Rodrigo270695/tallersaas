...
export default function AuthSplitLayout({ children, title, description }: AuthLayoutProps) {
    ...
    // (mi-taller.localhost), el título / descripción del saludo se
    // personalizan con el nombre del taller. En el panel central se
    // mantienen los textos genéricos definidos por la página.
    const tenantTitle = tenant
        ? (tenant.nombre_comercial || tenant.razon_social).trim() + '.'
        : title;
    const tenantDescription = tenant
        ? 'Ingresa con tu cuenta para gestionar órdenes de trabajo, clientes y caja.'
        : description;
    const headerBrand = tenant ? tenant.nombre_comercial || tenant.razon_social : brandName;
        ...
        <div className="relative isolate flex min-h-svh flex-col overflow-hidden bg-background text-foreground">
            ...
            <main className="relative z-10 flex flex-1 items-center justify-center px-4 py-8 sm:py-12 lg:py-16">
                ...
                <article className="relative z-10 mx-auto w-full max-w-md">
                    <AuthGreeting title={tenantTitle} description={tenantDescription} />
