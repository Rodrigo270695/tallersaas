...
import type { CategoriaFilters, CategoriaProducto, CategoriaStats } from './types';
...
export default function Index({
    ...
    const { search, setSearch, isLoading, sort, setSort, setPerPage, applyFilter } =
        useDataTablePage<{ estado: CategoriaFilters['estado'] }>({
            ...
            storageKey: 'tallersaas.inventario-categorias.prefs',
    ...
    const openEdit = useCallback(
        (categoria: CategoriaProducto) => setModal({ type: 'edit', categoria }),
        [],
    );
    const openDelete = useCallback(
        (categoria: CategoriaProducto) => setModal({ type: 'delete', categoria }),
        [],
    );
    ...
    const columns = useMemo<DataTableColumn<CategoriaProducto>[]>(() => {
        const base: DataTableColumn<CategoriaProducto>[] = [
            ...
            },
            {
                ...
                header: 'Repuestos',
                cell: (row) => (
                    <span className="tabular-nums text-sm">{row.productos_count ?? 0}</span>
                ),
            },
            {
                key: 'activo',
    ...
    return (
        <>
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 sm:p-6">
                <PageHeader
                    title="Categorías"
                    description="Agrupa los repuestos (filtros, aceites, frenos…)."
                    stats={[
                        { label: 'Total', value: stats.total, variant: 'info', icon: Tags },
                        { label: 'Activas', value: stats.activas, variant: 'primary', icon: Tags },
                        { label: 'Filtros', value: filters.estado !== 'todas' ? 1 : 0, variant: 'warning', icon: Filter },
                        {
                            label: 'Coincidencias',
                ...
                <DataTable
                    ...
                    emptyState={
                        <EmptyState
                            ...
                            description="Crea la primera para organizar el almacén."
                            action={
                                canCreate ? (
                                    <Button
                                        type="button"
