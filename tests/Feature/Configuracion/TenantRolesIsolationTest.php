...
it('cambia permisos de admin_taller solo en el tenant actual', function (): void {
    ...
    expect($roleA->permissions()->pluck('name')->sort()->values()->all())
        ->toBe(['clientes.view', 'dashboard.view']);

    expect($roleB->permissions()->pluck('name')->sort()->values()->all())
        ->toBe($permsBBefore);
});

it('no lista roles de otro tenant en el catálogo del taller', function (): void {
    $this->otherTenant = Tenant::query()->create([
        'slug' => 'otra-'.Str::lower(Str::random(4)),
        'schema_name' => 'taller_otra_'.Str::lower(Str::random(4)),
        'razon_social' => 'Otro taller',
