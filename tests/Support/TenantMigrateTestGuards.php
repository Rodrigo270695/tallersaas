...
/**
 * Los tests que ejecutan `tallersaas:tenant-migrate` o `RefreshDatabase` contra PostgreSQL
 * pueden destruir datos o alterar `public.migrations` si usan la misma base que en
 * desarrollo (p. ej. `tallersaas`). Ver {@see TenantSchemaMigrator}.
 */
final class TenantMigrateTestGuards
{
    ...
    public static function pgsqlUsesDedicatedTestDatabase(): bool
    {
        ...
        return filter_var(env('TALLERSAAS_ALLOW_TENANT_MIGRATE_TESTS', false), FILTER_VALIDATE_BOOL);
    }
    ...
    public static function message(): string
    {
        ...
        return 'Protección: estos tests ejecutan migraciones destructivas (`migrate:fresh` y/o '
            .'`tallersaas:tenant-migrate`) que no deben correr contra la base `'.$db.'`. '
            .'Usa una base dedicada cuyo nombre termine en `_test` o `_testing` (ej. `tallersaas_test`), '
            .'o define `TALLERSAAS_ALLOW_TENANT_MIGRATE_TESTS=true` en `.env` solo si aceptas ese riesgo.';
    }
}
